      ...
      <div class="detail-container">
        ...
        <div class="detail-info" id="vehicleDetail">
          ...
          ${vehicle.city ? `
            ...
            ${vehicle.seller_verified && (vehicle.seller_profile?.dealership_address || vehicle.seller_profile?.instagram) || vehicle.contact_phone || vehicle.seller_profile?.phone ? `
              <div class="seller-contact-actions">
                ...
                ` : ''}
                ${(vehicle.contact_phone || (vehicle.seller_profile?.phone && vehicle.seller_profile?.show_phone !== false)) && vehicle.status !== 'sold' ? `
                  <a href="https://wa.me/${escapeHtml((vehicle.contact_phone || vehicle.seller_profile.phone).replace(/[\s\-\(\)]/g,'').replace(/^\+/,''))}?text=${encodeURIComponent(`Hola, te contacto desde AutoVenta.online por el siguiente anuncio: https://autoventa.online/?vehicle=${vehicle.id}`)}" target="_blank" rel="noopener" class="btn btn-primary" style="background:#25D366;border:none;width:100%;margin-top:0.5rem;">
